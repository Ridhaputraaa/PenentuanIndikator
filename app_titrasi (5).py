    ...
    if metode != "── PILIH ──":
        ...
        with col_fp1:
            volume_labu = st.number_input(
                ...
                step=1,
                format="%d",
                help="Kosongkan / biarkan 0 untuk memakai fp standar = 100mL / 25mL = 4."
            )
        with col_fp2:
        ...
        if st.button("Hitung Konsentrasi"):
            if volume > 0:
                ...
                if volume_labu > 0 and volume_pipet > 0:
                    fp = volume_labu / volume_pipet
                    fp_keterangan = f"fp masukan pengguna ({volume_labu}mL / {volume_pipet}mL)"
                else:
                    fp = 100 / 25
                    fp_keterangan = "fp standar (100mL/25mL)"

                hasil = massa / (fp * BE * volume)
                st.write("**Rumus:**")
                st.code(formula)
                st.write("**Perhitungan:**")
                st.write(f"Hasil = {massa:.1f} mg / ({fp:.4f} × {BE} {unit_be} × {volume:.2f} mL)")
