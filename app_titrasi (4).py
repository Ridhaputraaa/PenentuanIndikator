...
if fitur == "MENGHITUNG STANDARISASI LARUTAN":
    ...
    if metode != "── PILIH ──":
        massa = st.number_input("Massa Zat Primer (mg)", min_value=0.0, format="%.4f")
        volume = st.number_input("Volume Titran / Larutan Standar Sekunder (mL)", min_value=0.0, format="%.2f")
        fp_input = st.number_input(
            "Faktor Pengali / fp (opsional)",
            min_value=0.0,
            value=0.0,
            format="%.4f",
            help="Kosongkan / biarkan 0 untuk memakai fp standar = 100mL / 25mL = 4. Isi sendiri jika volume pengenceran berbeda."
        )

        if st.button("Hitung Konsentrasi"):
            if volume > 0:
                if "NaOH" in metode or "KMnO₄" in metode:
                    BE, unit, label, unit_be = 63.0, "N", "Normalitas", "mg/mgrek"
                elif "HCl" in metode:
                    BE, unit, label, unit_be = 190.7, "N", "Normalitas", "mg/mgrek"
                elif "Tiosulfat" in metode:
                    BE, unit, label, unit_be = 49.04, "N", "Normalitas", "mg/mgrek"
                elif "EDTA" in metode:
                    BE, unit, label, unit_be = 100.09, "M", "Molaritas", "mg/mmol"

                fp_label = "BE" if unit == "N" else "BM"
                formula = f"{unit} = mg zat primer / fp (100mL/25mL) × {fp_label} zat primer × mL titran"

                fp = fp_input if fp_input > 0 else (100 / 25)
                fp_keterangan = "fp standar (100mL/25mL)" if fp_input <= 0 else "fp masukan pengguna"

                hasil = massa / (fp * BE * volume)
                st.write("**Rumus:**")
                st.code(formula)
                st.write("**Perhitungan:**")
                st.write(f"Hasil = {massa:.4f} mg / ({fp:.4f} × {BE} {unit_be} × {volume:.2f} mL)")
                st.caption(f"Keterangan: fp = {fp:.4f} ({fp_keterangan}); {fp_label} = {BE} {unit_be}")
...
st.markdown(
    ...
    <div class="custom-white-box" style="text-align:center; max-width: 400px; margin: 0 auto;">
        <span style='color: #0b3c5d !important; font-weight: 900 !important; font-size: .9em;'>
            ⚛️ Penentuan Indikator Titrasi &nbsp;|&nbsp; Kelompok 5 Kelas 1D Politeknik AKA Bogor
        </span>
    </div>
    """,
    unsafe_allow_html=True,
